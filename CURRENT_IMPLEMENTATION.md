...
**最終更新**: 2026-07-17（確定仕様v1.0 実装後）

**警告**: このファイルは引き継ぎ時点のスナップショット。実装を変更したら更新すること。
また、このファイルを鵜呑みにせず、**必ず実際のコードと照合**すること。

> ## 【2026-07-17】確定仕様v1.0 で以下が追加/変更された（詳細は `docs/SPEC.md` v0.4）
> - **追加**: 半キャラ累計3発→弱り／弱り正面ブラスト（撃破=場外KO・生存=ダウン）／ダウン本実装（3秒→起き上がり2秒弱り）／ジャンププレス／空中頭突き（走行中ジャンプ・強い敵に通る唯一の技）／向き規則（背面被弾）／ザコ4種（`EnemyMain.EnemyType`）／S4ボス号令＋ロープ走行＋直角カウンター／取り巻き周回／技名ポップアップ／headless自動テスト（`Scripts/Dev/CombatSim.gd`）
> - **変更**: ロープダッシュ倍率 2.0→1.25（走行中は連打間隔半減）／強い(Angry)は正面も無効化
> - **廃止**: ザコの自動怒り（HP40%/15秒周期）／ザコのロープ往復（ロープを走るのはボスだけ）／かすりの弱り化（かすりは小ダメのみのレア技）
> - 以下の本文は v0.3 時点の記述が残っている。矛盾したら上記とSPEC v0.4が正
