...
> ## 【2026-07-17 更新（Fable 5）】この文書の§3以降の仕様記述は v0.3 時点のもの
> - **確定仕様v1.0 を実装済み**（commit `8aa61af`〜）。現在の正: `docs/SPEC.md` v0.4 と `GAME_DESIGN_BRUSHUP.md` ★★確定仕様v1.0
> - 大枠: 半キャラ3発→弱り→正面ブラスト→ダウン→プレスのループ／強い敵は頭突きのみ／ザコ4種／S4号令＋ボスロープ走行＋直角カウンター／ザコ自動怒り廃止
> - 旧F-3（2往復停止）は**破棄**（無限往復が確定仕様）。Phase B のステージ個別改修は本実装に統合済み
> - headless自動テスト: `Godot4.7 --headless --path . -- stage=N sim=combat|boss|clear`（`Scripts/Dev/CombatSim.gd`）
> - 残: 人間の実プレイ検証とバランス調整のみ
