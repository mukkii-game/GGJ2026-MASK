# HANDOFF.md — デスクトップ側AIへの引き継ぎ

**作成日**: 2026-07-13  
**目的**: ノートPC（Cursor）→ デスクトップPC（Cursor）への完全な開発引き継ぎ
...
2. **このファイルを最後まで読む**
3. 引き継ぎ文書群を順に読む（`CLAUDE.md` に記載の順番で）
4. **`docs/SPEC.md` を読む**（正式仕様書）
5. **実際のコードを読んで照合する**（文書を鵜呑みにしない）
...
| ファイル | 状態 |
|----------|------|
| `Scenes/Player/Scripts/PlayerMain.gd` | staged (M) |
| `Scripts/DebugContactOverlay.gd` | unstaged ( M) |

これらの変更が引き継ぎコミットに含まれているか確認すること。
