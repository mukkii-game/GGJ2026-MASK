...
## TD-09: AudioManagerのサウンドプール上限 → **プール拡張の到達不能バグを修正済み（2026-07-13）**

**旧状況**: `max_players` を8→16に拡張したが、`play_sound()` の `available_player` 初期値が `audio_players[0]` になっていたため「空きが見つからずnullなら新規作成」という拡張コードに実質到達せず、常に3〜16体のプール内から探すだけで、全員再生中のときは`audio_players[0]`を問答無用で上書きしていた（拡張の意味がなかった）。

**修正**: `available_player` の初期値を `null` にし、「空きを探す→なければ上限未満で新規作成→それでも無ければ`audio_players[0]`にフォールバック」の順に修正。これで実際に16プールまで使われるようになった。
