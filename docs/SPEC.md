...
|---|---|---|---|
| S1 | ジョバーのみ（初期3・cap6・10秒補充） | なし（殲滅） | 半キャラ3発→青→正面ブラストの基本ループ |
| S2 | ジョバー＋ガブリ交互 | メロンナ: 逃走→コーナー追い詰め（従来） | 正面ブラスト・ガブリ場外ボウリング・終盤の取り巻き周回 |
...
- `StageController._setup_normal_params()`: `initial_count: 3`, `max_count: 6`, `spawn_interval: 10.0`
