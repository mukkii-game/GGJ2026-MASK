# SPEC: Body-check Arena（v0.5 / 実装反映: 2026-07-23 新戦闘方針）

> **v0.5 の大枠**（詳細は各セクション。採用方針は `GAMEPLAY_DECISION_BRIEF.md`）:
> 敵状態は **普通/弱り/強い＋ダウン**。弱りは**パワーエサのみ**（15〜20秒に1回）。
> **半キャラ**でザコは倒してよい。**かすり**＝両者スピン＋敵2.5秒ダウン → **ジャンプでフライングボディ**。
> **赤（怒り）**は真っ赤＋湯気。後ろ半キャラのみ有効／通常半キャラは被ダメ／かすり可。
> **ボス**はHP0で死亡せずダウン → ジャンプでQTE（失敗はHP1再開）。低HPでトップロープ攻撃約2回。
> プレイヤーロープは**左右のみ**。ザコ移動は5種＋ペア編成。空中頭突き・半キャラ3発弱りは廃止。
...
- **走行中（ロープ往復 or Nダッシュ）は連打間隔が半減（0.1秒）**（威力倍率はロープ中1.25倍＝`ROPE_DASH_DAMAGE_MULT`）。
- **弱り蓄積は廃止**（旧: 半キャラ3発→弱り）。ザコは半キャラ連打だけで倒してよい。
- 毎 tick で `PLAYER_ATTACK_HIT` を再生（通常 vol 0、走行系は vol 2）。
- **怒り（赤）中**: **後ろからの半キャラのみ有効**。それ以外の半キャラはプレイヤーが被ダメ＋ノックバック。
- **効果（ノックバック）**:
...
- **効果**: 両者スピンスピン演出。敵に小ダメージ＋**約2.5秒ダウン**（`notify_graze_hit` → `enter_down(2.5)`）。赤中でも可能。
- 0.4秒クールダウン。連続ヒットなし。

### 3.4 ジャンプ（回避＋フライングボディ）
- **空中頭突きは廃止**。ジャンプは回避とダウン追撃に特化。
- **ダウン中の敵**の上に着地 → **フライングボディアタック 40ダメージ**（技名「フライングボディ！」）。
  - 成功時は着地せず **即座に再ジャンプ**（敵をジャンプ台にした連続フライングボディ）。
  - 連続するたび **滞空・高さ・空中移動が伸びる**（威力は40固定。最大7段）。マット着地／失敗着地でリセット。
- **立っている敵**の上に着地 → **プレイヤーが吹き飛ぶ**（攻撃失敗）。
- **ボスHP0ダウン中**に着地 → **QTE開始**（`request_finisher_qte`）。再ジャンプなし。

### 3.4.1 描画順（奥行き）
- **キャラの足元Yのみ**で決定（画面上側＝奥＝低い z）。ジャンプの見た目浮き・ダウン回転のAABBは使わない。
- **ダウン中**は固定帯 `DRAW_Z_DOWNED_BASE`（約 -2800）へ置き、立ちキャラ（足元Y帯）より**常に奥**。※旧 `DRAW_Z_DOWN_PENALTY=8000` は Godot の z_index 範囲（±4096）外になり代入が無視されていた。
- トップロープ滞空攻撃のみ例外で加算ボーナス（最前面）。
- `z_as_relative = false`。敵親ノード `NPCs` は `Node2D`（CanvasItem）としてプレイヤーと同レイヤで比較。
...
#### Angry（=「強い」・v0.5）
- 見た目が**真っ赤**＋湯気パーティクル（`STATE_ANGRY_MODULATE` / `AngrySteam`）
- **後ろからの半キャラのみ有効**。通常の半キャラはプレイヤー被ダメ。
- **正面接触も無効**：敵は無傷、プレイヤーだけ8ダメ＋160pxノックバック（§3.1-3）
- **かすりは可能**（2.5秒ダウンへの逃げ道）
- 移動速度 **1.35倍**・敵攻撃のダメージ **1.5倍**
- **発生**: ザコは周期怒り（約10〜16秒隙→7秒赤）。ヒートマン発熱。S4号令。S3/S4ボス周期＋S3はHP40%以下。

#### Weak（=「弱り」・v0.5）
- 見た目が青色（`STATE_WEAK_MODULATE`）
- 移動速度 **0.5倍**、攻撃ダメ0
- **正面接触＝ブラスト**（§3.1-1）
- **発生: パワーエサのみ**（15〜20秒に1回・8秒）。半キャラ蓄積・踏み・起き上がり弱り・ガブリ常時は廃止。

#### Down（ダウン・寝 / v0.5）
- スプライト90度回転。**約2.5秒**（かすり等）。プレス／フライングボディ対象。フライングボディではダウン時間を延長しない。
- **描画**: ダウン中は誰よりも奥（`DRAW_Z_DOWNED_BASE`。立ちキャラの足元Y帯より常に低い）。
- トップロープ空振り時のダウンは **1秒**。
- **ボスHP0**: `enter_finisher_down` → ジャンプでQTE。失敗時HP=1再開。
- 発生: かすり／弱りブラスト生存／直角カウンター／トップロープ空振り 等。

判定箇所: `PlayerMain._body_contact()`、`EnemyMain` 状態API、`PlayerJumpState`（フライングボディ／フィニッシュ）。
...
- リング（マット）内の移動範囲を固定し、**ロープ外には出られない**（クランプ）
  - Player のマット範囲（`PlayerMain.gd`）：左 280 / 右 1000 / 上 **138** / 下 614（上は背景ロープに食い込まない余裕）
  - Enemy のマット範囲（`EnemyMain.gd`）：左 296 / 右 984 / 上 **138** / 下 614（片側 16px 狭い＝敵がロープ端に張り付かない）
- ロープ見た目の前後関係（`ArenaMat`）：
  - `ring_bg.png` にロープが焼き付いている。ColorRect の上下3本重ね描きは **位置ずれのため一旦オフ**（背景絵のみ表示）
  - 左右ロープは背景絵のまま
...
|---|---|---|---|
| S1 | ジョバー。**1匹教習→倒すとゴング→約2秒→「やっちまえー」→群れ（ノルマ8）** | なし（撃破ノルマ） | 半キャラ基本 |
| S2〜S4 | ザコ先行→ボスはポスト待機後降臨。撃破はHP0ダウン→ジャンプ→QTE（失敗HP1）。クリア時ゴング3連→約2秒後クリア画面 | 各ボス | 面ごとのギミック |
| S2 | ジョバー＋ガブリ交互 | メロンナ: **開幕は青ポール上で高みの見物**（当たり判定なし・`is_perched`・モーション超低速・赤青なし）→ザコ全滅（最低4秒）or 20秒で**くるくる回転しながらゆっくり降臨**＋吹き出し→逃走→コーナー追い詰め | 正面ブラスト・ガブリ場外ボウリング・終盤の取り巻き周回 |
...
### 発生条件
- ボスHPが0になると即QTEではなく **ダウン（フィニッシュ待ち）**
- ダウン中にプレイヤーがジャンプ着地するとQTE開始

### 結果
- **成功**: ボス撃破・残ザコ逃走・ステージクリア
- **失敗**: ボス **HP=1** で戦闘再開
...
- **ロープ跳ね返り**: プレイヤーは**左右のみ**。ザコのサイン/ヨタヨタは四辺で反転。`ArenaMat.bend_rope()` でたわみ。跳ね返り開始時にスプライト回転リセット＋向き合わせ（後ろ向き走行バグ対策）。
...
- **Win（現行）**: `StageController.gd` がステージクリア時に `Scenes/UI/StageClear.tscn` へ遷移。  
  ...
  - クリア絵は **書き文字なし一枚絵を優先**（`iron_mask_clearN` / `iron_mask_titleN_notext` があればそれ。無ければ `iron_mask_titleN`）。  
