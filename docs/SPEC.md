...
- **判定条件**: ズレが **32以上 58未満** かつ、プレイヤーが敵の方向へ十字入力を入れている場合（上下左右どちらも可）。
- 旧: 左右のみ半キャラ・上下はかすり／半キャラ上限52。かすりが出やすすぎたため半キャラ帯を拡大。
- **効果（ダメージ）**:
...
- **判定条件**: ズレが **58以上 64未満** かつ、プレイヤーが敵の方向へ十字入力を入れている場合（かすり帯は約6px＝旧12pxの半分）。
...
- **ダウン中**は固定帯 `DRAW_Z_DOWNED_BASE`（約 **-180**）へ置き、立ちキャラ（足元Y帯）より奥、かつ **ArenaMat（z=-200）より手前**。※旧 -2800 はマットの下に描画されて消えたように見えた。
...
- **BGM（2026-07-13）**: `AudioManager` の専用 `BGMPlayer` で管理（Autoload・シーン外）。  
  ...
  - 登場画面: 攻略テキストパネルは非表示。キャラごとに **吹き出し一言**（画面右上・`StageIntro.gd`）。S2「メロンパンの魅力を知りな！」／S3「今のアタシは／バージョン７よ！」（改行）／S4「イーロン反論オブジェクション！」
